...
def update_category(id):
    ...
    """
    response = session.post(f"{BASE_URL}/{id}", data=updated_data, headers=headers)
    response.close()
...
def test_category_change_performance():
    process = psutil.Process()

    for count in ITERATIONS:
        print(f"Updating {count} categories...")
    ...
    plt.tight_layout()
    plt.savefig("C/graphs/category_change_graph.png")
    plt.close()

if __name__ == "__main__":
    server = start_api_server()
    try:
        test_category_change_performance()
    finally:
