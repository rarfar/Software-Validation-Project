...
def test_todo_delete_performance():
    process = psutil.Process()

    for count in ITERATIONS:
        print(f"Deleting {count} todos...")
        ...
        for todo_id in ids:
            session.delete(f"{BASE_URL}/{todo_id}")
    ...
    plt.tight_layout()
    plt.savefig("C/graphs/todo_delete_graph.png")
    plt.close()

if __name__ == "__main__":
    server = start_api_server()
    try:
        test_todo_delete_performance()
    finally:
